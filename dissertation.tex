...
The code generation have bring a lot of difficulties to the project and this is due to the complexity of creating a compiler which can be found:
\begin{itemize}
\item processing the LISS code only one time and  processing from the first character to the last one.
\item saving some informations in the cache memory relatively for generating the code in the correct order.
\item creating a linked-list in MIPS assembly code relatively to the sequence type in order to be workable.
\item creating mips code with alignment address.
\item choosing a certain architecture for generating the code (using some regisers or not, using the stack or the heap relatively to a context).
\item applying some algorithm for the use of some structure available in the project.
\end{itemize}
