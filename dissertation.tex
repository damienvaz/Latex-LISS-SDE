...
And those instructions uses, each one, a mnemonic to represent each low-level machine instruction or operation.
...
In Table ~\ref{lst:r-type_format}, the instruction is composed by one mnemonic and three operands (note that each operand is represented by \$ sign).

The \textit{OP} word is the mnemonic for the particular instruction in MIPS (it could be an ~\textit{add} for adding, ~\textit{sub} for substracting...) and the ~\textit{rd}, ~\textit{rs}, ~\textit{rt} are the registers used and available in MIPS.

Take a note that ~\textit{rd} is the destination register and ~\textit{rs}, ~\textit{rt} are the source registers.

In a mathematical way, those instructions have the following semantics (see below).
...
Let's see an example of one R-type instruction in MIPS for a better understanding of that context (see Listing ~\ref{lst:example_r-type}).

\begin{lstlisting}[caption={Example of a R-type instruction},label={lst:example_r-type}]
  add $t1, $t1, $t2
\end{lstlisting}

The instruction shown in Listing ~\ref{lst:example_r-type}, means that register \$t1 (rs register according to the format of R-type instruction) shall be added (due to \textit{add} mnemonic) to register \$t2 (rt register according to the format of the R-type instruction) and their sum (the result) stored in \$t1 register (rd register according to the format of the R-type instruction).

This is equivalent by saying (in a mathematical form):

\begin{align*}
  OP\ rd,\ rs,\ rt &\iff rd\ =\ rs\ OP\ rd\\
...
Converting the R-type instruction into machine code will perform the following way (see Table ~\ref{tbl:r-type_binary_machine_code}).
