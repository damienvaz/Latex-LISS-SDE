...
None of this could have been made without their unconditional help.
	...
	% Add abstracts (en,pt) ---------------------------
	\chapter*{Abstract}
	This dissertation is about \textbf{programming languages}---their design and formal specification with a
grammar---\textbf{compilation}---including parsing, contextual validation for semantic checking,
and assembly code generation--- \textbf{source program edition}---to help language beginners to
write easily correct programs.

To be more specific, 
the aim of this master work is to write an attribute grammar (AG) for LISS, the Language of Integers Sequences and Sets,
by using  AnTLR compiler generator tool, in order to implement the static semantic analyzer and  generate
MIPS assembly code. 
To execute the generated code, MARS---the MIPS Assembler and Runtime Simulator---will be explored.
LISS is a toy language, designed twenty years ago in the Department of Informatics at UM to teach compiler courses;
LISS is a small imperative programming language that offers special features to work with arrays,
dynamic sequences and sets---these constructors pose complex and interesting compilation problems.

However, the main objective of this masters' work is the implementation of an integrated development environment (IDE) supported by a specific LISS editor that drives the creation of any source program free of syntactic errors.

Using  LISS context free grammar,
it is possible to create a Syntax-Directed Editor (SDE) in order to provide the typical help
of a structured editor which controls the writing of a source program according to the language syntax as defined
by the underlying  grammar. After synthesizing the correct source text it is possible to compile and run it
without exiting the editing environment.
	
	\cleardoublepage
	\chapter*{Resumo}
	Esta dissertação trata de linguagens de programação --- o seu design e a sua especificação formal com uma gramática --- compilação --- incluindo análise, validação contextual para verificação semântica e geração de código de baixo nível --- edição do programa fonte - para ajudar os iniciantes a escrever correctamente e facilmente programas.
	%-->
    Para ser mais específico, o objetivo deste trabalho principal foi de escrever uma gramática de atributos (AG) para LISS, a Linguagem de Sequências e Conjuntos Inteiros, usando a ferramenta geradora de compiladores AnTLR, para implementar o analisador semântico estático e gerar código de baixo nível para MIPS. Para executar o código gerado, o MARS --- the MIPS Assembler and Runtime Simulator --- serão explorados. LISS é uma linguagem pedagógica, desenhada há vinte anos atrás no Departamento de Informática da UM para ensinar cursos de compilação; LISS é uma pequena linguagem de programação imperativa que oferece recursos especiais para trabalhar com matrizes, sequências dinâmicas e conjuntos - esses construtores apresentam complexos e interessantes problemas de compilação.
%No entanto, o principal objetivo deste trabalho de mestres é a implementação de um ambiente de desenvolvimento integrado (IDE) apoiado por um editor LISS específico que impulsiona a criação de qualquer programa fonte livre de erros sintáticos.
%Usando LISS gramática livre de contexto, é possível criar um Editor Direto de Sintaxe (SDE) para fornecer a ajuda típica de um editor estruturado que controla a escrita de um programa de origem de acordo com a sintaxe de linguagem como definido pela gramática subjacente. Depois de sintetizar o texto fonte correto, é possível compilar e executá-lo sem sair do ambiente de edição.
...
%%%%%%%%%%%%%%%%%
	...
	\chapter{Introduction}
Computing is the activity concerned with solving problems using a computer to help the end-users in many of their daily tasks, professional or private, facilitating their life.
To solve problems using the computer, the developers have to instruct the computer writing programs.
To create programs, the programmers must use a formal language, general purpose or domain specific,
more or less complex, but anyway it must be recognized by the computer.
On account of this, in this context of computing, \textbf{programming languages} play a crucial role.

Developing programs is a difficult task; analyzing problems, designing the algorithm to solve them,
programming and debugging software requires a big effort and a large amount of time.
This huge complexity demands high-level programming languages (HLPL);
machine-code or assembly are no more sufficient to write so complex programs.
This explains why programming languages have evolved so much in the last decades and in so many
directions, supporting diverse approaches to problem solving (or different programming paradigms), 
and offering different data types, control structures and operators.

When compared with low-level languages, such as assembly, HLPLs offer precise semantics and abstraction mechanisms, 
that allow to hide details concerned with the computer architecture or its working principle---they
must be closer to programmers than the machine.
HLPLs must be easy to learn and simple to read, expressive, concise (non-ambiguous), consistent, scalable.

More than well designed, powerful programming languages,
software developers require powerful tools to help them increasing their productivity.
The most important tools are: \textbf{compilers} to generate lower-level code (machine code) from the high-level source code (the input program written in an high-level programming language);
\textbf{editors} to create the source code; 
\textbf{debuggers}, \textbf{code-analyzers}, etc. to help on fixing programming errors .
And to make easier and safer the programmers work, environments that integrate these tools,
improving their usage and performance, are also most desirable.

Although there are many different programming paradigms and programming languages, 
as well as nice tools for software development,
there is always space for further research in this area,
looking for new language/tool features.
On one hand it is a challenging activity,
on the other hand it comes out with fresh ideas, better algorithms, data structures and products 
that contribute to make the programmers life easier.

In this scenario, a group of researchers on compilers and programming languages implementation, at Universidade do Minho (UM),
designed years ago an imperative programming language called \textbf{LISS} (Language for Integers, Sequences and Sets)
that, although small, includes at a high level of abstraction some specific features (like arrays, dynamic sequences and sets defined in comprehension) that improve the language expressivity and poses interesting compilation problems.
The formal specification of LISS, as well as its compilation to different virtual machines, as been the subject of 
previous projects.
However people in the  Language Processing group at UM thinks that this language can be the object of
new experiences and further research --- this was the basis for the present masters' work.

\section{Objectives}
The main aim of this project is the development of an editor for LISS 
following the concept of a Syntax Directed Editor (SDE).

LISS language will be specified by an attribute grammar, that is known as the most adequate formalism
to specify the syntax and semantics of programming languages.
Thereafter the compiler generator tool AnTLR will be used because it is one of the most important compiler
generators nowadays, it supports attribute grammars with inherited and synthesized attributes
(following and L-attribution schema), and it generates a powerful and efficient LL(k) parser written in Java. 

The compiler generated by AnTLR will compile LISS programs into MIPS assembly code (lower-level source code).
MIPS is an interesting virtual machine based on a RISC architecture, used in many projects;
moreover it is supported by two good simulators that will be used to run the code generated by the LISS compiler.

To be more concrete, the objectives of this project are:
\begin{enumerate}
\item 
Create an AnTLR version of the existing CFG grammar for LISS language.

\item
Extend the LISS CFG to an AG in order to specify the language static semantics (validation of the contextual conditions),
and its dynamic semantics, i.e., the generation of MIPS assembly code.
To verify the correctness of the assembly code generated, MARS (a simple but effective MIPS simulator)  will be selected to provide all the tools for checking it.

\item
Develop the desired Structural-Editor, SDE, using the grammar produced for AnTLR.
It will be implemented in  JavaFX because AnTLR is implemented in Java and, also, uses Java as target language.
Additionally, an IDE, shall be created as the main interface for the desired syntax-directed editor.

\item
Finally, exhaustive and relevant tests will be made to the tool created; the outcomes will be analyzed and discussed.
\end{enumerate}

\section{Research Hypothesis}

It is possible to synthesize a complete source program, ready to be compiled and executed,
selecting the appropriate alternative language constructors and writing literals in the right
positions in a special editor guided by the source language structure, or syntax.
...
In this chapter, after introducing  the  area of Programming Languages Processing, to provide some context/motivation,
the objectives of  this masters' project were defined.

Chapter 2 introduces the definition of a formal language and a grammar, followed by some context of the LISS language and its structure (data types, blocks, statements and subprograms) and finally, discussing the evolution of the LISS language.

Chapter 3 defines the MIPS architecture (cpu data formats, memory usage, instructions formats and memory management) and different MIPS simulator used for the project.

Chapter 4 discusses about the compiler development by introducing some context of AnTLR technology, followed by the lexical, syntatical and semantic analysis, and the code generation.

Chapter 5 introduces the concept of a syntax directed editor and the conception of the syntax directed editor for the project.

The dissertations ends in Chapter 6 with a summary, a short discussion on the project outcomes, and 
...
Last, the ~\textit{value(s)} field refers to the value of the type associated.
...
Beside those instructions, some other instructions are sequences of instructions and they are called as pseudo instructions (see in Table ~\ref{tbl:pseudo_instructions}).
...
%And this is due for having a good consistency regarding to the information available in the symbol table.
...
Last, the compiler checks if the ~\textit{identifier} has the correct category (it must belong to the category ~\textit{Function}). If it doesn't have then it must throw the number 9 error message (see Table ~\ref{tbl:types_error_message}).
...
%So, as said previously, the compiler will throw both the number 12 and 13 error messages.
...
finally, the compiler will move the sum to the appropriated register to call the ~\textit{cons()} function (line 14) and reload the information that he stored previously to the stack (related with the second argument of the ~\textit{cons()} function) (line 15 to 17); last, it will process the call of the ~\textit{cons()} function and store the result into the variable ~\textit{sequence2} (line 18 to 20).
...
that are summarized in Table ~\ref{tbl:sequence_function_mips}:

\begin{table}[h!]
\centering
\caption{Sequence predefined operations}
\label{tbl:sequence_function_mips}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Mode}             & \textbf{\begin{tabular}[c]{@{}c@{}}Name of the sequence \\ operator in LISS\end{tabular}} & \textbf{\begin{tabular}[c]{@{}c@{}}Name of the sequence \\ operator in MIPS\end{tabular}} \\ \hline
...
Finally, it will print the output of the execution of the liss code to the window number 3 (see Figure ~\ref{fig:LISS-SDE_structure}).
