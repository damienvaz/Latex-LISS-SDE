...
To complement the Symbol Table and deal easily with variables declared in scopes with level greater than 0 (inside function); we use a stack depicted in Figure ~\ref{fig:stack_symbol_table}, where we can see two components: ~\textit{levelStackSP} and ~\textit{stackSP}.
...
\end{figure}
 ...
 ~\textit{levelStackSP} is a stack where the index is associated with the scope level of a LISS program.
The value stored on each index store information about the index of the  ~\textit{stackSP} array.
Meanwhile the value in each index of the ~\textit{stackSP} array store information about the space that the compiler required when a stack operation was used in a LISS program.

By creating that structure, the compiler will use an algorithm for finding a specific position relatively to a variable in the stack faster, instead of creating multiple lines of MIPS assembly code for finding the position. In this case, it is done an optimization in executing a LISS program faster.

For example, in Listing ~\ref{fig:stack_symbol_table}, if the compiler needs to figure out which address is a variable in scope level 2. It will, throw the ~\textit{levelStackSP}, access to the index 2 and get the value 4. Then with the value 4, he will access it throw the index of the  ~\textit{stackSP} array and gets the address number 64.
...
The mechanism is simple, every functions whose arguments have the possibility of calling a function or are used in an expression must use the stack available in MIPS architecture.
...
\begin{itemize}
\item Scope level equals to zero
\end{itemize}
