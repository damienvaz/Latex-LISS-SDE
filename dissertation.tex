...
The variable must be unique/single (see Listing ~\ref{lst:variable_name_single}).
...
Variable names contains only letters and numbers, or underscore sign however, notice that the first character of the variable name must be a letter (lower and upper case).
...
As in a traditional imperative language (let's compare 'C language'), if we don't take the habit of implementing the variable always in a certain part of the code, it becomes confusing. This makes the life harder to the programmer in understanding the code when the code is quite long/big.
...
As MIPS solved those problems, it was primarly used for embedded systems and video games consoles (which requires a lot of arithmetic computation) until late 2006, when it came to computers.
...
The architecture MIPS has 32 registers and there are some conventions to use those registers correctly.
Table ~\ref{tbl:compiler_register_usage} illustrates the meaning of those registers:
...
\multicolumn{1}{|c|}{\$t0 - \$t7}     & \multicolumn{1}{l|}{\$8 - \$15}        & \multicolumn{1}{l|}{temporaries registers}                                                                                                  & \multicolumn{1}{c|}{No}                                                                        \\ \hline
\multicolumn{1}{|c|}{\$s0 - \$s7}     & \multicolumn{1}{l|}{\$16 - \$23}       & \multicolumn{1}{l|}{saved temporaries registers}                                                                                            & \multicolumn{1}{c|}{Yes}                                                                       \\ \hline
\multicolumn{1}{|c|}{\$t8 - \$t9}     & \multicolumn{1}{l|}{\$24 - \$25}       & \multicolumn{1}{l|}{temporaries registers}                                                                                                  & \multicolumn{1}{c|}{No}                                                                        \\ \hline
...
The Table ~\ref{tbl:compiler_register_usage} is composed by 4 columns:
...
\item \textit{Number} column refers to the number of the register associated in MIPS. It can been also interacted with the instructions available in MIPS. But the nomenclature associated doesn't give so much context of the register (instead of \textit{Name} column).
...
And those instructions use, each one, a mnemonic to represent each low-level machine instruction or operation.
Let's see deeper in the next section.

\subsection{MIPS R-type}

R-type instruction refer a register type instruction (it is the most complex type in MIPS).
The idea behind of that instruction is to operate with only and only registers.

This type use the following format in MIPS (see Listing ~\ref{lst:r-type_format}).

\begin{lstlisting}[caption={R-type instruction format},label={lst:r-type_format}]
  OP rd, rs, rt
\end{lstlisting}

In Listing ~\ref{lst:r-type_format}, the instruction is composed by one mnemonic designed by \textit{OP} and three operands designed by ~\textit{rd} (destination register), ~\textit{rs} (source register), ~\textit{rt} (another source register).

In a mathematical way, the R-type instruction format has the following semantic (see below).
...
The instruction shown in Listing ~\ref{lst:example_r-type}, means that register \$t1 shall be added (due to \textit{add} mnemonic) to register \$t2 and their sum (the result) stored in \$t1 register (note that each operand is represented by \$ sign).

This is equivalent by saying (in a mathematical form):
...
Converting the R-type instruction into machine code will be performed by the following way (see Table ~\ref{tbl:r-type_binary_machine_code}).
...
Let's explain each of the fields of the Table ~\ref{tbl:r-type_binary_machine_code}.

\begin{itemize}
\item \textbf{opcode} derivated from "operation code"; it indicates which kind of operation it is. Thus, not only the \textit{opcode} is needed only, but also the \textit{function} column is needed too (for R-type instructions). Note that every R-type instruction has the \textit{opcode} set to the value 0. The \textit{opcode} field is 6 bits long (bit 31 to bit 26).
...
Let's see an example of a R-type instruction to his transformation to machine code in Table ~\ref{tbl:r-type_transformation2machine_code}.
...
\begin{align*}
  add\ \$t0,&\ \$t0,\ \$t1\\
  &\makebox[\widthof{${}={}$}]{$\Downarrow$} \\
  \texttt{0x01}\texttt{094020}\ &(in\ hexadecimal)\\
  &\makebox[\widthof{${}={}$}]{$\Downarrow$}
\end{align*}


%tabela do hexadecimal para binary
\vspace{-8mm}
\begin{table}[h]
\centering
\begin{tabular}{l|l|c|c|c|c|c|c|c|c|}
\cline{2-10}
\tikzmark{a}&\textbf{Hexadecimal} & 0    & 1    & 0    & 9    & 4    & 0    & 2    & 0    \\ \cline{2-10}
\tikzmark{b}&\textbf{Binary}      & 0000 & 0001 & 0000 & 1001 & 0100 & 0000 & 0010 & 0000 \\ \cline{2-10}
\end{tabular}
...
In Table ~\ref{tbl:r-type_transformation2machine_code}, woot
