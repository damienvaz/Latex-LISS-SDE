...
So, in this case, we need to know in which address the first element of the sequence is stored and this pass by knowing the address of the first element of the sequence. However this address must be stored on a place that can be known and this goes by storing that address to the variable name associated.

Also the size of an address in the MIPS architecture is 4 bytes, then in that case the variable must be 4 bytes long and that is why we choose the type ~\textit{.word}.
