...
Relatively to those operators, only tree operators use one instruction and the others uses a logic of mechanism for having the same behaviour which requires more than one instruction. Notice that this is due to the fact that MIPS architecture doesn't have all the relation instruction implemented.
