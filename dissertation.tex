...
MIPS architecture has 32 registers and there are some conventions to use those registers correctly.
...
I-type instruction is an instruction which operate with an immediate value and a register value and the immediate value cannot be longer than 16 bits.

This type use the following format, mostly, in MIPS (see Listing ~\ref{lst:I-type_format}).

\begin{lstlisting}[caption={I-type instruction format},label={lst:I-type_format}]
  OP rd, rs, IMM
\end{lstlisting}

In Listing ~\ref{lst:I-type_format}, the instruction is composed by one mnemonic designed by ~\textit{OP}, two operands designed by ~\textit{rd} (destination register), ~\textit{rs} (source register) and one number value designed by ~\textit{IMM}.

In a mathematical way, the I-type instruction format has the following semantic (see below).

\begin{lstlisting}
  rd = rs OP IMM
\end{lstlisting}

To understand better this instruction, let's see an example of one I-type instruction in MIPS (se Listing ~\ref{}).

\begin{lstlisting}[caption={Example of an I-type instruction},label={}]
  li $v0, 4
\end{lstlisting}
