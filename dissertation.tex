...
In Listing ~\ref{lst:r-type_format}, the instruction is composed by one mnemonic designed by \textit{OP} and three operands desgined by ~\textit{rd} (destination register), ~\textit{rs} (source register), ~\textit{rt} (another source register).
...
\begin{itemize}
\item \textbf{opcode} derivated from "operation code"; it indicates which kind of operation is it. Thus, not only the \textit{opcode} is needed only, but also the \textit{function} column is needed too (for R-type instructions). The \textit{opcode} field is 6 bits long (bit 31 to bit 26).
