...
To understand better the table ~\ref{tbl:syscall_mips}, we need to give some explanation of it.
The ~\textit{service} column gives us the context of the service; the ~\textit{code} column explains which value we need to set to the register \$v0 (associated to the service wished); the ~\textit{arguments} column load the arguments values depending the service and lastly, the ~\textit{result} column give some informations about the return value of the service (if available or not).
...
Also, notice that, every instructions who are shown in the tables, are instructions which were used for the project (almost all of them).

\section{MIPS Memory Management}

MIPS has the possibility to controlling and coordinating the computer memory by two ways:
\begin{enumerate}
\item stack
\item heap
\end{enumerate}
...
For this project, we used two nice freely simulators:
\begin{itemize}
\item MARS simulator
\item SPIM simulator
\end{itemize}
...
They debug and execute MIPS assembly code but only MARS simulator has the possibility to write some MIPS assembly code in their IDE.
