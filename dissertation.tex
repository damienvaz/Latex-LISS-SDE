...
We need to create a system which will take the information regarding the array declared to generate the appropriated code (see Figure ~\ref{fig:access_array_structure}). So we created the following structure (see in Listing ~\ref{lst:access_array_structure}).
...
Creating variables in a inner scope (with a level greater than 0) is necessary when those variables are declared inside of a function, let's see the one exemplified in Listing ~\ref{lst:variable_declarations_greater_than_0}.
...
\begin{itemize}
\item First, the return address of the function is allocated.
\item Then, the arguments of the function are allocated.
\item Finally, the local variables.
\end{itemize}
