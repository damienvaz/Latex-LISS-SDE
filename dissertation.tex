...
Let's explain throw the Figure ~\ref{fig:global_symbol_table_liss} how it does work the symbol table in LISS.
...
For our project we implemented the symbol table with an HashMap where the key is an identifier and the value is a LinkedList of some informations related to the identifier.
...
The identifier (associated to ~\textit{String} in ~\ref{lst:symbol_table_liss_java_implementation}) must be unique and the ~\textit{LinkedList} must have the idea of an ordered list, that's why we choosed to use a ~\textit{HashMap} data structure associated to a ~\textit{LinkedList}.
...
The ~\textit{TYPE} category contains informations about which primative type is available in LISS. In our cases, they are known by : set, integer, sequence and boolean. And the ~\textit{TYPE} category contains information about their fixed size of each type in MIPS.

The ~\textit{VAR} category contains informations about the variables declared in LISS code. It might be an integer, boolean, array, set or a sequence variable.

Lastly, the ~\textit{FUNCTION} category contains informations about the subprograms created in a LISS code.

There are 5 types recognised in LISS:
\begin{enumerate}
\item Integer
\item Boolean
\item Set
...
Each time that the identifier is inserted in the HashMap, it insert the information related to that identifier and in this case, it is the type of the identifier.
Care that ~\textit{LinkedList} has the notion of being an ordered list, and this is very important for the symbol table because it says which level of the scope the identifier was found.
...
And regarding to each type, it contains some informations which are different for each type.
Let's explain in the next table
...
\subsubsection{Semantic system for LISS}
